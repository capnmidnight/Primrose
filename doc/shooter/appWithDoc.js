...
var cube = range(6, function (i) {
  return "../images/space" + i + ".jpg";
});
...
var shots = [];
var TEMP = v3();
var baseVectorSize = 3;
var asteroidColor = 0x7f7f7f;
var shotSoundType = "sawtooth";
var asteroid = [box(baseVectorSize).colored(asteroidColor).named("asteroid1"), [], []];
var sounds = null;
var nextSound = 0;
var explosion = null;
var checker = shooter();
    ...
    for (var y = 0; y < 2; ++y) {
      for (var z = 0; z < 2; ++z) {
        var sz = baseVectorSize * 0.5 / s,
            b = box(sz).colored(asteroidColor).named("asteroid" + s + "_" + x + "_" + y + "_" + z);
...
function updateObj(obj, dt) {
  if (obj.velocity) {
    TEMP.copy(obj.velocity).multiplyScalar(dt).add(obj.position);
    obj.position.copy(TEMP);
    if (obj.sound) {
      obj.sound.at(obj.position.x, obj.position.y, obj.position.z, obj.velocity.x, obj.velocity.y, obj.velocity.z);
    }
  }
}
...
env.addEventListener("update", function (dt) {
  for (var i = 0; i < blocks.length; ++i) {
    var block = blocks[i];
    TEMP.copy(env.input.head.position).sub(block.position);
    var dist = TEMP.lengthSq();
    block.velocity.add(TEMP.normalize().multiplyScalar(0.5 / dist));
    updateObj(block, dt);
  }
      ...
      for (var j = 0; j < hits.length; ++j) {
        ...
        if (d < v) {
          ...
          if (sounds) {
            ...
            snd.play().at(block.position.x, block.position.y, block.position.z, block.velocity.x, block.velocity.y, block.velocity.z);
          }
          ...
          if (block.nextSize < asteroid.length) {
            ...
            for (var l = 0; l < newBlocks.length; ++l) {
              ...
              newBlock.velocity = Primrose.Random.vector(-1, 1).add(block.velocity);
...
function shoot(evt) {
  ...
  var block = null;
  if (n >= shots.length) {
    block = box(0.01, 0.01, 0.05).colored(0xffff7f, {
      unshaded: true
  ...
  block.velocity.copy(evt.pointer.picker.ray.direction).multiplyScalar(10);
  block.age = 3;
  block.lookAt(TEMP.copy(block.position).add(block.velocity));
  block.visible = true;
  block.sound = env.music.getOsc(shotSoundType).on(30, 0.25).at(block.position.x, block.position.y, block.position.z, block.velocity.x, block.velocity.y, block.velocity.z).on(5, 0.25, 0.5, true).off(0.51);
}
