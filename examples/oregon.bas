...
4  REM
5  REM Run the program at any time by hitting CTRL+SHIFT+ENTER on your IBM PC keyboard, CMD+OPT+E on your Apple ][e. Type commands when prompted. The game mostly uses numeric commands, but there is a shooting mini-game that requires you to quickly type a given word, so pay attention for when it comes up.
6  REM
