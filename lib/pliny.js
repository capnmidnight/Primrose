...
( function ( module ) {
    ...
    format: function ( name ) {
      ...
      if ( obj ) {
        ...
        output += "\n\n" + [
          ...
          "enumerations",
          "records",
          "author" ].map( formatters.checkAndFormatField.bind( this, obj ) )
  ...
  formatters.html = {
    ...
    formatArray: function ( obj, arrName, arr ) {
      ...
      if ( arrName === "description" ) {
        var temp = arr.slice();
        output += "<p><details><summary>" + temp.shift() + "</summary>" + temp.join( "</p><p>" ) + "</details></p>";
      }
      else {
    ...
    shortDescription: function ( topLevel, p ) {
      ...
      output += "><code>" + ( topLevel ? p.fullName : p.name ) + " <span class=\"" + ( p.type ? "type" : "fieldType" ) + "\">" + ( p.type || p.fieldType ) + "</span>";
