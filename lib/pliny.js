  ...
  formatters.html = {
    ...
    formatField: function ( obj, p, o ) {
      ...
      if ( o instanceof Array ) {
        ...
        output += "<p>" + this.endocumate( o ) + "</p>";
      }
      else if ( p === "code" || p === "result" ) {
        output += "<h4>" + p + "</h4>" + this.endocumate( o );
      }
      else if ( p === "diagram" ) {
        output += "<img src=\"" + o + "\">";
      }
    ...
    endocumate: function ( str ) {
      var pres = [ ];
      return str.replace( /&/g, "&amp;" )
          .replace( /</g, "&lt;" )
          .replace( />/g, "&gt;" )
          .replace( /``([^`]+)``/g, function ( str, match ) {
            var name = "&CODE_SNIP" + pres.length + ";";
            pres.push( match );
            return name;
          } )
          ...
          .split( "\n" )
          .join( "<br>" )
          .replace( /&CODE_SNIP(\d+);/g, function ( str, match ) {
            return "<pre>" + pres[match] + "</pre>";
          } );
    },
    ////
    ...
    examplesFormat: function ( obj, arr ) {
      ...
      for ( var i = 0; i < arr.length; ++i ) {
        var ex = arr[i],
            desc = this.endocumate( ex.description );
        output += "<div><h4><a href=\"#" + ex.id + "\">" + ex.name + "</a></h4><p>" +
            desc + "</p><h5>Code</h5>" + this.endocumate( ex.code ) +
            "<h5>Result</h5>" + this.endocumate( ex.result ) +
            "</div>";
      }
      return output;
    },
