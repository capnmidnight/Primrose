...
( function ( module ) {
  ...
  formatters.html = {
    ...
    endocumate: function ( str ) {
      var pres = [ ];
      return "<p>" + str
          .replace( /(``?)([^`]+)\1/g, function ( str, match1, match2 ) {
            var name = "&PRE_SNIP" + pres.length + ";",
                tag = match1 === "``" ? "pre" : "code",
                content = match2.replace( /&/g, "&amp;" ).replace( /</g, "&lt;" ).replace( />/g, "&gt;" );
          ...
          .replace( /^([^\n]+)\n=+\n/mg, "<h1>$1</h1>" )
          .replace( /^([^\n]+)\n-+\n/mg, "<h2>$1</h2>" )
          .replace( /(#{1,6})\s*([^#\n]+)\s*#*\n/g, function ( str, match1, match2 ) {
            var h = match1.length;
            return "<h" + h + ">" + match2 + "</h" + h + ">";
          } )
          .replace( /((\*|-|\+|\d+\.)\s+[^\n]+\n)+/g, function ( str ) {
    ...
    examplesFormat: function ( obj, arr ) {
      ...
      for ( var i = 0; i < arr.length; ++i ) {
        var ex = arr[i];
        output += "<div>" +
            "<h4><a href=\"#" + ex.id + "\">" + ex.name + "</a></h4>" + this.endocumate( ex.description ) +
            "<h5>Code</h5>" + this.endocumate( ex.code ) +
