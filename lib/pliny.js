...
( function ( module ) {
  ...
  var database = {
    type: "database",
    name: "Documentation Top Level",
    fullName: "Documentation Top Level",
    ...
    format: function ( name ) {
      ...
      if ( obj ) {
        ...
        output += "\n\n" + [
          ...
          "code",
          "result",
          "references" ].map( formatters.checkAndFormatField.bind( this, obj ) )
  ...
  formatters.html = {
    format: function ( name ) {
      var obj = resolveBag( database, name ),
          id = obj.fullName.replace( /(\.|:)/g, "_" ).replace( / /g, "" );
      return "<article id=\"" + id + "\" class=\"" + obj.fieldType + "\">" + formatters.format.call( formatters.html, obj ) + "</article>";
    },
    /////
    ...
    descriptionFormat: function ( obj, arr ) {
      var temp = arr.slice();
      return "<details><summary>" + temp.shift() + "</summary><p>" + temp.join( "</p><p>" ) + "</p></details>";
    },
    ////
    ...
    examplesFormat: function ( obj, arr ) {
      var output = "<details><summary>Examples</summary>";
      for ( var i = 0; i < arr.length; ++i ) {
        output += this.format( arr[i].fullName );
      }
    ...
    shortDescription: function ( topLevel, p ) {
      ...
      if ( p.type || p.fieldType !== "example" ) {
        output += " <span class=\"" + ( p.type ? "type" : "fieldType" ) + "\">" + ( p.type || p.fieldType ) + "</span>";
      }
  ...
  formatters.console = {
    format: function ( name ) {
      console.log( formatters.format.call( formatters.console, name ) );
    },
    /////
    // Puts together a string that describes a top-level field out of a documentation
    // object.
