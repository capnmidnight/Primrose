...
( function ( module ) {
  ...
  // These types are allowed to have their values automatically set.
  var topLevelTypes = [
    "namespace",
    "class",
    "function",
  ...
  ///
  function fillBag ( name, rootObject ) {
  ...
  function analyzeObject ( fieldType, parentName, info ) {
    ...
    if ( info ) {
      ...
      // Fix parent references for prototype-level class objects.
      if ( /\.prototype$/.test( parentName ) ) {
        parentName = parentName.substring( 0, parentName.length - 10 );
        info.onPrototype = true;
      }
      ...
      // Fill in the only default value we can guess at.
      if ( info.fieldType === "namespace" && typeof info.value === "undefined" ) {
        info.value = {};
      }
      ...
      var parentBag = fillBag( parentName, database );
  ...
  function copyObjectMetadata ( info ) {
    var fullName = ( info.parent && ( info.parent + "." ) || "" ) + info.name,
        bag = fillBag( fullName, database );

    // Make sure we aren't setting the data for a second time.
    if ( !bag.fieldType ) {

      // If the user gave us a value, write it out to the world.
      if ( typeof info.value !== "undefined" &&
          ( topLevelTypes.indexOf( info.fieldType ) > -1 || info.onPrototype ) ) {
        var parentName = info.parent;
        if ( info.onPrototype ) {
          parentName += ".prototype";
        }
        var parent = fillBag( parentName, window );
        if ( info.fieldType === "property" ) {
          Object.defineProperty( parent, info.name, info.value );
        }
        else {
          parent[info.name] = info.value;
        }
      }
  ...
  formatters.html = {
    ...
    formatField: function ( obj, p, o ) {
      ...
      else if ( obj.fieldType === "page" && p === "description" ) {
        output += endocumate( o );
      }
  ...
  function loadFiles ( files, success, failure ) {
    if ( files && files.length > 0 ) {
      if ( typeof success === "undefined" ) {
        ...
        var __loadFiles = function ( i ) {
          if ( i >= files.length ) {
            success();
          }
          else {
            Primrose.HTTP.getText( files[i], function ( txt ) {
              var name = "";
