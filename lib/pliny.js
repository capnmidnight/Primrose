      ...
      for ( var i = 0; i < arr.length; ++i ) {
        var ex = arr[i],
            ...
            .replace( /``([^`]+)``/g, function ( str, match ) {
              return "<pre>" + formatters.html.escape( match ) + "</pre>";
            } )
            .replace( /`([^`]+)`/g, "<code>$1</code>" );
        output += "<div><h4><a href=\"#" + ex.id + "\">" + ex.name + "</a></h4><p>" +
            desc + "</p><h5>Code</h5><pre class=\"source-code\">" + formatters.html.escape( ex.code ) +
            "</pre><h5>Result</h5><pre>" + formatters.html.escape( ex.result ) +
            "</pre></div>";
      }
      return output;
    },
