...
THREE.VREffect = function ( renderer, vrHMD ) {
  ...
  this.render = function ( scene, camera, renderTarget, forceClear ) {
    ...
    // render left eye
    cameraLeft.projectionMatrix = this.FovToProjection(
        this.left.recommendedFieldOfView, true,
        camera.near, camera.far );
    camera.matrixWorld.decompose( cameraLeft.position, cameraLeft.quaternion,
        cameraLeft.scale );
    cameraLeft.translateX( this.left.eyeTranslation.x );
    renderer.setViewport( this.left.renderRect.x, this.left.renderRect.y,
        this.left.renderRect.width, this.left.renderRect.height );
    renderer.setScissor( this.left.renderRect.x, this.left.renderRect.y,
        this.left.renderRect.width,
        this.left.renderRect.height );
    renderer.render( scene, cameraLeft, renderTarget, forceClear );

    // render right eye
    cameraRight.projectionMatrix = this.FovToProjection(
        this.right.recommendedFieldOfView,
        true, camera.near, camera.far );
    camera.matrixWorld.decompose( cameraRight.position, cameraRight.quaternion,
        cameraRight.scale );
    cameraRight.translateX( this.right.eyeTranslation.x );
    renderer.setViewport( this.right.renderRect.x, this.right.renderRect.y,
        this.right.renderRect.width, this.right.renderRect.height );
    renderer.setScissor( this.right.renderRect.x, this.right.renderRect.y,
        this.right.renderRect.width, this.right.renderRect.height );
