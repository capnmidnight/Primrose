...
import { createUtilityCanvas, isCanvas, isHTMLCanvas, resizeContext, setContextSize } from "@juniper-lib/dom/dist/canvas";
import { border, padding as cssPadding, display, getMonospaceFonts, height, overflow, perc, width } from "@juniper-lib/dom/dist/css";
import { onMouseDown, onMouseMove, onMouseOut, onMouseOver, onMouseUp, onTouchEnd, onTouchMove, onTouchStart } from "@juniper-lib/dom/dist/evts";
import { Canvas, InputText, TextArea, HtmlRender, elementClearChildren, getElement } from "@juniper-lib/dom/dist/tags";
import { TypedEvent, TypedEventTarget } from "@juniper-lib/events/dist/TypedEventTarget";
import { isApple, isFirefox } from "@juniper-lib/tslib/dist/flags";
import { isDefined, isNullOrUndefined, isString } from "@juniper-lib/tslib/dist/typeChecks";
import { Cursor } from "./Cursor";
import { Delayer } from "./Delayer";
import { Grammar, JavaScript, grammars } from "./Grammars";
...
let focusedControl = null, hoveredControl = null;
const publicControls = [], controls = [], wheelScrollSpeed = 4, vScrollWidth = 2, scrollScale = isFirefox() ? 3 : 100, optionDefaults = Object.freeze({
    readOnly: false,
    multiLine: true,
    wordWrap: true,
    scrollBars: true,
...
}), elements = new WeakMap(), ready = (document.readyState === "complete"
...
export class Primrose extends TypedEventTarget {
    ...
    constructor(options) {
        ...
        this.longPress = new Delayer(1000);
        ...
        if (isNullOrUndefined(this.surrogate)) {
            HtmlRender(document.body, this.surrogate = TextArea(ID("primroseSurrogate"), display("none")));
        }
        //>>>>>>>>>> VALIDATE PARAMETERS >>>>>>>>>>
        options = options || {};
        ...
        //<<<<<<<<<< VALIDATE PARAMETERS <<<<<<<<<<
        //>>>>>>>>>> KEY EVENT HANDLERS >>>>>>>>>>
        this.keyDownCommands = Object.freeze(new Map([
            ["CursorUp", () => {
        ...
        if (isNullOrUndefined(options.element)
            ...
            this.canv = Canvas(width(perc(100)), height(perc(100)));
            this.element.appendChild(this.canv);
            this.element.removeAttribute("tabindex");
            HtmlRender(this.element, display("block"), cssPadding(0), border("2px inset #c0c0c0"), overflow("unset"));
        }
    ...
    set2DMouseEvents(canv) {
        HtmlRender(canv, onMouseOver(() => this.readMouseOverEvent()), onMouseOut(() => this.readMouseOutEvent()), onMouseDown((evt) => this.readMouseDownEvent(evt)), onMouseMove((evt) => this.readMouseMoveEvent(evt)), onMouseUp(() => this.readMouseUpEvent()));
    }
    //<<<<<<<<<< MOUSE EVENT HANDLERS <<<<<<<<<<
    ...
    set2DTouchEvents(canv) {
        HtmlRender(canv, onTouchStart((evt) => this.readTouchStartEvent(evt)), onTouchMove((evt) => this.readTouchMoveEvent(evt)), onTouchEnd((evt) => this.readTouchEndEvent(evt)));
    }
}
